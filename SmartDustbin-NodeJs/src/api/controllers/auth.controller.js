...
const {createSendOtp, createVerifyOtp, createUser,createVerifyWebOtp}=require('../services/auth.service');
...
module.exports={
    ...
    verifyWebOtp:(req,res)=>{
        ...
        createVerifyWebOtp(body,function(err,result){
            if(err){
                return res.json(error(500,"Database connection error"))
            }else{
                return res.json(success(result.code,result.message,result.data));
