    ...
    sendOtp:(data,callback)=>{
        ...
        db(function(err,connection){
            if(err){
                ...
                connection.query("SELECT * FROM consumer_user WHERE consumer_phone="+consumerPhone,function(err,results){
                    if(err){
                        connection.release();
                        return callback({code:500,error:err});
                    }else{
                        ...
                        if(results.length>0){
                            ...
                            if(otpExpiry>nowTime){
                                ...
                                newOtpSql="UPDATE consumer_user SET otp='"+otp+"', otp_expiry='"+expiryTime+"' WHERE consumer_phone="+consumerPhone;
                            }
                        }else{
                            newOtpSql="INSERT INTO consumer_user(consumer_phone,otp,otp_expiry) VALUES('"+consumerPhone+"','"+otp+"','"+expiryTime+"')";
                        }
            ...
            if(err){
                ...
                connection.query("SELECT * FROM consumer_user WHERE consumer_phone="+consumerPhone,function(err,results){
                    if(err){
                        ...
                        if(consumerUser.consumer_name==null||consumerUser.consumer_name==""){
                            connection.release();
                            return callback(null,{code:201,message:"User account doesn't exist",data:null});
                        }

                        connection.query("UPDATE consumer_user SET token='"+token+"' WHERE consumer_phone="+consumerPhone,function(err,results){
                            if(err){
                                connection.release();
                                return callback({code:500,error:err});
                            }else{
                                ...
                                var responseData={
                                    ...
                                    consumer_profile:constants.CONSUMER_PROFILE+consumerUser.consumer_profile,
