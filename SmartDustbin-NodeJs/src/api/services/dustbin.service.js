    ...
    getAllDustbins:(data,callback)=>{
        ...
        db(function(err,connection){
            if(err){
                ...
                connection.query("SELECT dustbins.*,categories.category_name FROM dustbins LEFT JOIN categories ON dustbins.category_id=categories.id WHERE dustbins.producer_id="+producerId,function(err,results){
                    if(err){
                        connection.release();
                        return callback({code:500,error:err});
                    }else{
    ...
    getAllCategories:(data,callback)=>{
        ...
        db(function(err,connection){
            if(err){
             ...
             connection.query("SELECT *,concat('"+constants.CATEGORY_URL+"',category_image) as category_image FROM categories ORDER BY id",function(err,results){
                if(err){
                    connection.release();
                    return callback({code:500,error:err});
                }else{
        ...
        dbPromise(async function(err,connection){
            if(err){
                ...
                try{
                    ...
                    if(parseFloat(wasteWeight)+parseFloat(currentCapacity)>=dustbin.total_capacity){
                        connection.rollback();
                        return callback({code:500,error:err});
                    }
                    ...
                    const dustbinContentData={
                        ...
                        closing_quantity:parseFloat(wasteWeight)+parseFloat(currentCapacity),
                        depth:dustbinDepth,
                        closing_depth:parseFloat(dustbinDepth)+parseFloat(currentDepth),
                        created_at:nowTime
                    }
                    ...
                    const [dustbinOverall]=await connection.query("UPDATE dustbins SET current_capacity=? WHERE id=?",[parseFloat(wasteWeight)+parseFloat(currentCapacity),dustbinId]);
    
                    const responseData={
                        producer_id:producerId,
                        new_capacity:parseFloat(wasteWeight)+parseFloat(currentCapacity),
                        dustbin_id:dustbinId
                    }

                    connection.commit();
                    return callback(null,{code:200,message:"Waste data updated successfully",data:responseData});
                }catch(error){
                    if(connection) await connection.rollback();
                    return callback({code:500,error:error});
                }finally{
    ...
    getDustbinWasteDistribution:(data,callback)=>{
        ...
        dbPromise(async function(err,connection){
            if(err){
                ...
                connection.release();
                return callback(null,{code:200,message:"Dustbin waste distribution fetched successfully",data:wasteDistribution});
            }
        })
    },

    getDustbinWebDetail:(data,callback)=>{
        const dustbinId=data.dustbin_id;
        ...
        db(function(err,connection){
            if(err){
                ...
                connection.query("SELECT dustbins.*,categories.category_name FROM dustbins LEFT JOIN categories ON dustbins.category_id=categories.id WHERE dustbins.id="+dustbinId,function(err,results){
                    if(err){
                       return callback({code:500,error:err}); 
                    }else{
                        if(results.length>0){
                            return callback(null,{code:200,message:"Dustbin details fetched successfully",data:results[0]});                            
                        }else{
                            return callback(null,{code:200,message:"Dustbin details not available",data:null});
                        }
                    }
                });
            }
